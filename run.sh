...
sleep 30s
$HADOOP_HOME/bin/hadoop --config $HADOOP_CONF_DIR dfs -copyFromLocal $FOLDER/input/*.txt Input
echo "Input files on HDFS"
$HADOOP_HOME/bin/hadoop --config $HADOOP_CONF_DIR dfs -ls Test
echo "Run the hadoop job"
$HADOOP_HOME/bin/hadoop --config $HADOOP_CONF_DIR jar $HADOOP_HOME/contrib/streaming/hadoop*streaming*.jar -file $FOLDER/mapper.py -mapper $FOLDER/mapper.py -reducer org.apache.hadoop.mapred.lib.IdentityReducer -input /user/$USER/Input/* -output /user/$USER/Output
echo "Output files on HDFS"
$HADOOP_HOME/bin/hadoop --config $HADOOP_CONF_DIR dfs -ls Output
echo "Copying output locally"
$HADOOP_HOME/bin/hadoop --config $HADOOP_CONF_DIR dfs -copyToLocal Output/* $FOLDER/output/
sleep 30s
echo
